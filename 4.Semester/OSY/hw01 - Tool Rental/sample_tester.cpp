...
void                                   CCompanyTest::solvedPack                ( AProblemPack                          pack )
{
  for ( auto problem : pack -> m_Problems )
  {
    ...
    if ( idx >= g_Problems . size () )
      throw std::invalid_argument ( "isSolvedPack: called too many times" );
    
    if ( g_Problems . begin () [idx] != problem )
      throw std::invalid_argument ( "isSolvedPack: order not preserved" );

    if ( problem -> m_MaxProfit != g_Results[idx] )
      throw std::invalid_argument ( "isSolvedPack: invalid result" );
  }
}
//-------------------------------------------------------------------------------------------------------------------------------------------------------------
